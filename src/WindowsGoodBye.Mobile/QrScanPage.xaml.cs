...
public partial class QrScanPage : ContentPage
{
    ...
    private async void ProcessQrCode(string qrData)
    {
        ...
        try
        {
            // Parse QR payload: wingb://pair?{base64}|{ip1,ip2,...}|{relayUrl}|{pushAuthEnabledDefault}
            // See PairingSession.GenerateQrData for the authoritative format documentation (Fase 10,
            // docs/plan_push_auth_v2.md). The last two segments were added by Fase 10 and are always
            // present (possibly empty) in QRs generated by an up-to-date PC — this app and the Service
            // are developed/deployed together, so no version negotiation is attempted here.
            var afterPrefix = qrData[Protocol.PairQrPrefix.Length..];
            var parts = afterPrefix.Split('|');

            var payloadBase64 = parts[0];

            string[] pcIpAddresses = parts.Length > 1 && parts[1].Length > 0
                ? parts[1].Split(',', StringSplitOptions.RemoveEmptyEntries)
                : Array.Empty<string>();
            if (pcIpAddresses.Length > 0)
                System.Diagnostics.Debug.WriteLine($"[QrScan] PC IPs from QR: {string.Join(", ", pcIpAddresses)}");

            string? relayUrl = parts.Length > 2 && parts[2].Length > 0
                ? Uri.UnescapeDataString(parts[2])
                : null;

            // Defaults to true (matches DeviceInfo.PushAuthEnabled's own default) for QRs generated
            // before Fase 10 or missing the segment for any other reason.
            var pushAuthEnabledDefault = parts.Length <= 3 || parts[3] != "0";
            ...
            var pairedPc = new PairedPc
            {
                ...
                // Fase 10: synced from the QR at pairing time instead of waiting for the first
                // "auth_challenge" FCM payload (FcmService.TryUpdateRelayUrl) to opportunistically
                // learn RelayUrl, or for the user to discover PushAuthEnabled's default some other way.
                RelayUrl = relayUrl,
                PushAuthEnabled = pushAuthEnabledDefault
            };
